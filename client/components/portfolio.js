...
class Portfolio extends Component {
  ...
  async componentDidMount() {
    ...
    if (stocks.length) {
      ...
      stockSymbolsStr = this.props.stocks
        .map(currStock => currStock.stockSymbol)
        .join(',')
    ...
    if (stocks.length && latestStockData[stocks[0].stockSymbol]) {
      sortedStocks = stocks
        ...
        */
          ...
          currStock.currValue = Number(
            (currStock.numOfShares * latestPrice).toFixed(2)
          )
          //How much has the stock changed in value since the user bought the stock
          currStock.totalChange = Number(
            (currStock.currValue - currStock.totalInvested).toFixed(2)
          )
...
const PortfolioHeaders = ({handleSort, sortOn, sortOrder}) => {
  return (
    <ul className="portfolio-header">
      <li onClick={() => handleSort('stockSymbol')}>
        Stock Symbol <span hidden={sortOn !== 'stockSymbol'}>{sortOrder}</span>
      </li>
      <li onClick={() => handleSort('latestPrice')}>
        Latest Price <span hidden={sortOn !== 'latestPrice'}>{sortOrder}</span>
      </li>
      <li onClick={() => handleSort('todaysChange')}>
        Today's Gain/Loss{' '}
        <span hidden={sortOn !== 'todaysChange'}>{sortOrder}</span>
      </li>
      <li onClick={() => handleSort('totalChange')}>
        Total Gain/Loss{' '}
        <span hidden={sortOn !== 'totalChange'}>{sortOrder}</span>
      </li>
      <li onClick={() => handleSort('currValue')}>
        Current Value <span hidden={sortOn !== 'currValue'}>{sortOrder}</span>
      </li>
      <li onClick={() => handleSort('numOfShares')}>
        Quantity <span hidden={sortOn !== 'numOfShares'}>{sortOrder}</span>
      </li>
      <li onClick={() => handleSort('costBasis')}>
        Cost Basis <span hidden={sortOn !== 'costBasis'}>{sortOrder}</span>
      </li>
      <li onClick={() => handleSort('totalInvested')}>
        Total Invested{' '}
        <span hidden={sortOn !== 'totalInvested'}>{sortOrder}</span>
      </li>
      <li onClick={() => handleSort('createdAt')}>
        Date of Purchase{' '}
        <span hidden={sortOn !== 'createdAt'}>{sortOrder}</span>
      </li>
    </ul>
  )
}
...
const PortfolioStocks = ({idx, currStock}) => {
  ...
  */
  let todaysChangeColor =
    todaysChange > 0 ? 'gain' : todaysChange < 0 ? 'loss' : 'no-change'
  let totalChangeColor =
    totalChange > 0 ? 'gain' : totalChange < 0 ? 'loss' : 'no-change'
  return (
    <ul className={`portfolio-row ${idx % 2 === 0 ? 'shade-alternate' : ''}`}>
      <li name="stockSymbol" value={currStock.stockSymbol}>
        {currStock.stockSymbol}
      </li>
      <li className={todaysChangeColor}>
      ...
      Percent change is (B - A)/A * 100
        */}
      <li className={todaysChangeColor}>
        ${(todaysChange * currStock.numOfShares).toFixed(2)}
        <br />
        {(todaysChange / open * 100).toFixed(2)}%
      </li>
      {/* 
      Change in value for this stock in the users portfolio since the stock was purchased, and the percent change
      Percent change is (B - A)/A * 100
        */}
      <li className={totalChangeColor}>
        ${totalChange}
        <br />
        {(totalChange / Number(currStock.totalInvested) * 100).toFixed(2)}%
      </li>
      <li>{`$${currValue}`}</li>
...
const mapDispatchToProps = dispatch => {
  return {
    loadStockQuotes: ownedStockSymbols =>
      dispatch(fetchPortfolioThunk(ownedStockSymbols)),
